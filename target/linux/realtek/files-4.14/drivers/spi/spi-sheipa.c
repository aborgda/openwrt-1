...
static int sheipa_spi_probe(struct platform_device *pdev)
{
	...
	spi->regs = devm_ioremap_resource(&pdev->dev, mem);
	if (IS_ERR(spi->regs)) {
	...
	spi->auto_regs = devm_ioremap_resource(&pdev->dev, auto_mem);
	if (IS_ERR(spi->auto_regs)) {
		status = PTR_ERR(spi->auto_regs);
		goto err_unmap;
	}

	spi->max_freq = 0;
	/* Always ask for fixed clock rate from a property. */
	device_property_read_u32(&pdev->dev, "clock-frequency", &spi->max_freq);

	/* If there is separate clock, get the rate from it. */
	clk = devm_clk_get(&pdev->dev, NULL);
	if (!IS_ERR_OR_NULL(clk)) {
		/* Use the clock from clock source */
		status = clk_prepare_enable(clk);
		if (status)
			dev_warn(&pdev->dev, "could not enable clock source: %d\n", status);
	else
		spi->max_freq = clk_get_rate(clk);
	}
	...
	master->min_speed_hz = spi->max_freq/8;
	master->max_speed_hz = spi->max_freq;
...
static const struct of_device_id spi_sheipa_of_match[] = {
	{ .compatible = "realtek,sheipa-spi", },
	{ /* end of table */}
};
MODULE_DEVICE_TABLE(of, spi_sheipa_of_match);
...
static struct platform_driver sheipa_spi_driver = {
	...
	.driver = {
		   .name = DRIVER_NAME,
		   .owner = THIS_MODULE,
		   .of_match_table = spi_sheipa_of_match,
		   },
};
